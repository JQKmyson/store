...
def store_money():
    ...
    if digit == 1:
        ...
        sql = "updata bank set money = money + %s where account =%s"
        param = [money_one,accout_one]
        cursor.execute(sql,param)
        sql = "select money from bank where account = %s"
        param = [accout_one]
        cursor.exexcute(sql,param)
        sql = "select money from bank where account = %s"
        param = [accout_one]
        cursor.execute(sql,param)
        data = cursor.fetchone()
...
def bank_drawCash(account, password, cash):
    ...
    sql = "selcet * from banl where account = %s"
    param = [account]
    cursor.execute(sql,param)
    ...
    # 开始取钱
    # 判断转账金额账号是否充足
    sql = "select * from banl where account = %s and password = %s"
...
def bank_query(account, password):
    ...
    sql = "selcet * from bank where account = %S"
    param = [account]
    cursor.execute(sql,param)
    data = cursor.fetchall()
    if not data:
